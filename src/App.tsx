...
const convertBasicShapeToPath = (element: Element): string | null => {
  ...
  switch (tagName) {
    case "rect": {
      ...
      if (rx === 0 && ry === 0) {
        // 角丸なしの長方形
        return `M${x},${y} L${x + width},${y} L${x + width},${y + height
          } L${x},${y + height} Z`;
      } else {
        // 角丸長方形（簡略化）
        const r = Math.min(rx, ry, width / 2, height / 2);
        return `M${x + r},${y} L${x + width - r},${y} Q${x + width},${y} ${x + width
          },${y + r} L${x + width},${y + height - r} Q${x + width},${y + height
          } ${x + width - r},${y + height} L${x + r},${y + height} Q${x},${y + height
    ...
    case "circle": {
      ...
      const kr = k * r;
      return `M${cx},${cy - r} C${cx + kr},${cy - r} ${cx + r},${cy - kr} ${cx + r
        },${cy} C${cx + r},${cy + kr} ${cx + kr},${cy + r} ${cx},${cy + r} C${cx - kr
        },${cy + r} ${cx - r},${cy + kr} ${cx - r},${cy} C${cx - r},${cy - kr
    ...
    case "ellipse": {
      ...
      const krx = k * rx;
      const kry = k * ry;
      return `M${cx},${cy - ry} C${cx + krx},${cy - ry} ${cx + rx},${cy - kry
        } ${cx + rx},${cy} C${cx + rx},${cy + kry} ${cx + krx},${cy + ry
        } ${cx},${cy + ry} C${cx - krx},${cy + ry} ${cx - rx},${cy + kry} ${cx - rx
        },${cy} C${cx - rx},${cy - kry} ${cx - krx},${cy - ry} ${cx},${cy - ry
        } Z`;
    }
...
const convertToShape = (
  ...
  shouldNormalizeAspectRatio: boolean,
): string => {
          ...
          for (let i = 0; i < params.length; i += 6) {
            if (i + 5 < params.length) {
              if (command === "C") {
                ...
                currentY = params[i + 5];
                shapeParts.push(
                  `curve to ${formatCoordinate(currentX, currentY)} with ${formatCoordinate(cp1x, cp1y)
...
function App() {
  ...
  const processingResult = useMemo<ProcessingResult | null>(() => {
    if (!svgData) return null;

    try {
      const pathDataArray = extractPathsFromSVG(svgData.content);

      if (pathDataArray.length === 0) {
        return { shapeResult: "", error: "No path elements found in SVG" };
      }

      const svgDimensions = getSVGDimensions(svgData.content);
      const shapeValue = convertToShape(pathDataArray, svgDimensions, normalizeAspectRatio);
      return { shapeResult: shapeValue };
    } catch (err) {
      return { shapeResult: "", error: "Error processing SVG: " + (err as Error).message };
    }
  }, [svgData, normalizeAspectRatio]);
  ...
  const handleFileSelect = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file.name.toLowerCase().endsWith(".svg")) {
      setFileError("Please select an SVG file");
      setSvgData(null);
      return;
    }
    ...
    reader.onload = (e) => {
      ...
      setSvgData({ content, fileName: file.name });
    };
    reader.readAsText(file);
  };

  const copyToClipboard = () => {
    if (processingResult?.shapeResult) {
      navigator.clipboard.writeText(processingResult.shapeResult);
    }
  };
    ...
    <div className="app">
      ...
      <div className="aspect-toggle">
        <label>
          <input
            ...
            onChange={(e) => setNormalizeAspectRatio(e.target.checked)}
          />
          Normalize to 1:1 aspect ratio
        </label>
      </div>

      {(fileError || processingResult?.error) && (
        <div className="error">
          {fileError || processingResult?.error}
        </div>
      )}
      ...
      {svgData && (
        <div className="preview-section">
          <h3>SVG Preview: {svgData.fileName}</h3>
          <div
      ...
      {processingResult?.shapeResult && (
        <div className="result-section">
          <h3>CSS shape() Result:</h3>
          <div className="result-container">
            <pre className="result">{processingResult.shapeResult}</pre>
          ...
          <div className="usage-example">
            <h4>Usage Example:</h4>
            <pre>{`:root {
  --icon-${svgData?.fileName.replace(".svg", "")}: ${processingResult.shapeResult};
}
.element {
  clip-path: var(--icon-${svgData?.fileName.replace(".svg", "")});
  width: 1em;
  height: 1em;
          ...
          <div className="preview-clippath">
            ...
            <div className="clippath-demo-container">
              <div
                ...
                style={{
                  clipPath: processingResult.shapeResult,
                }}
              />
