...
function App() {
  ...
  const convertToShape = (pathDataArray: string[], svgDimensions: { width: number; height: number }): string => {
            ...
            if (i + 1 < params.length) {
              ...
              if (i === 0) {
                ...
                if (isFirstMove) {
                  ...
                  shapeParts.push(`move to ${formatCoordinate(currentX, currentY)}`);
                }
  ...
  const processSVG = (content: string) => {
    try {
      ...
      console.log("Original path data array:", pathDataArray);
